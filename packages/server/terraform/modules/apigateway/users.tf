...
# POST /vaults/{vaultId}/permissions/access/batch — batched access summaries
resource "aws_api_gateway_resource" "permissions_access_batch" {
  rest_api_id = aws_api_gateway_rest_api.vaultguard.id
  parent_id   = aws_api_gateway_resource.permissions_access.id
  path_part   = "batch"
}

resource "aws_api_gateway_method" "permissions_access_batch_post" {
  rest_api_id   = aws_api_gateway_rest_api.vaultguard.id
  resource_id   = aws_api_gateway_resource.permissions_access_batch.id
  http_method   = "POST"
  authorization = "COGNITO_USER_POOLS"
  authorizer_id = aws_api_gateway_authorizer.cognito.id
}

resource "aws_api_gateway_integration" "permissions_access_batch_post" {
  rest_api_id             = aws_api_gateway_rest_api.vaultguard.id
  resource_id             = aws_api_gateway_resource.permissions_access_batch.id
  http_method             = aws_api_gateway_method.permissions_access_batch_post.http_method
  integration_http_method = "POST"
  type                    = "AWS_PROXY"
  uri                     = var.perms_lambda_invoke_arn
}
