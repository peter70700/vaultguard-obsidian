...
# PUT /vaults/{vaultId}/files/deleted — write a root file literally named "deleted".
# API Gateway resolves the static sibling before the greedy {filePath+} route, so
# this explicit method preserves normal file semantics for that single filename.
resource "aws_api_gateway_method" "files_deleted_put" {
  rest_api_id   = aws_api_gateway_rest_api.vaultguard.id
  resource_id   = aws_api_gateway_resource.files_deleted.id
  http_method   = "PUT"
  authorization = "COGNITO_USER_POOLS"
...
resource "aws_api_gateway_integration" "files_deleted_put" {
  rest_api_id             = aws_api_gateway_rest_api.vaultguard.id
  resource_id             = aws_api_gateway_resource.files_deleted.id
  http_method             = aws_api_gateway_method.files_deleted_put.http_method
  integration_http_method = "POST"
  type                    = "AWS_PROXY"
  uri                     = var.files_lambda_invoke_arn
}

# DELETE /vaults/{vaultId}/files/deleted — soft-delete the root file named "deleted".
resource "aws_api_gateway_method" "files_deleted_delete" {
  rest_api_id   = aws_api_gateway_rest_api.vaultguard.id
  resource_id   = aws_api_gateway_resource.files_deleted.id
  http_method   = "DELETE"
  authorization = "COGNITO_USER_POOLS"
...
resource "aws_api_gateway_integration" "files_deleted_delete" {
  rest_api_id             = aws_api_gateway_rest_api.vaultguard.id
  resource_id             = aws_api_gateway_resource.files_deleted.id
  http_method             = aws_api_gateway_method.files_deleted_delete.http_method
  integration_http_method = "POST"
  type                    = "AWS_PROXY"
  uri                     = var.files_lambda_invoke_arn
}

# /vaults/{vaultId}/files-decrypted — sibling of /files for the per-file
# plaintext decrypt route (Phase 8). See infrastructure/lambda/files/handler.ts
# handleReadDecrypted.
resource "aws_api_gateway_resource" "files_decrypted" {
  rest_api_id = aws_api_gateway_rest_api.vaultguard.id
  parent_id   = aws_api_gateway_resource.vaults_id.id
  path_part   = "files-decrypted"
}

# Greedy child must be a LEAF — API Gateway forbids sub-resources under {path+}.
resource "aws_api_gateway_resource" "files_decrypted_path" {
  rest_api_id = aws_api_gateway_rest_api.vaultguard.id
  parent_id   = aws_api_gateway_resource.files_decrypted.id
  path_part   = "{filePath+}"
}

resource "aws_api_gateway_method" "files_decrypted_path_get" {
  rest_api_id   = aws_api_gateway_rest_api.vaultguard.id
  resource_id   = aws_api_gateway_resource.files_decrypted_path.id
  http_method   = "GET"
  authorization = "COGNITO_USER_POOLS"
  authorizer_id = aws_api_gateway_authorizer.cognito.id
  request_parameters = {
...
resource "aws_api_gateway_integration" "files_decrypted_path_get" {
  rest_api_id             = aws_api_gateway_rest_api.vaultguard.id
  resource_id             = aws_api_gateway_resource.files_decrypted_path.id
  http_method             = aws_api_gateway_method.files_decrypted_path_get.http_method
  integration_http_method = "POST"
  type                    = "AWS_PROXY"
  uri                     = var.files_lambda_invoke_arn
}
...
resource "aws_api_gateway_deployment" "vaultguard" {
  ...
  depends_on = [
    ...
    aws_api_gateway_integration.files_deleted_put,
    aws_api_gateway_integration.files_deleted_delete,
  ...
  triggers = {
    ...
    redeployment = sha1(jsonencode([
      ...
      aws_api_gateway_integration.files_deleted_put.id,
      aws_api_gateway_integration.files_deleted_delete.id,
