/** Dependency-free SHA-256 used for stable comparison hashes in every JS runtime. */
export function sha256Hex(value: string): string {
  if (typeof value !== "string" || hasUnpairedSurrogate(value)) {
    throw new TypeError("sha256Hex requires a string without unpaired Unicode surrogates.");
  }
  ...
  for (let offset = 0; offset < padded.length; offset += 64) {
    for (let index = 0; index < 16; index += 1) {
      schedule[index] = readUint32(padded, offset + index * 4);
    }
